...
<%
    ...
    if ("POST".equalsIgnoreCase(request.getMethod())) {
        ...
        // Fetch form inputs
        String appointmentDate = request.getParameter("appointmentDate"); // e.g., "2024-11-23"
        String startTime = request.getParameter("startTime");             // e.g., "08:00"
        String endTime = request.getParameter("endTime");                 // e.g., "09:02"
        ...
        // Combine appointmentDate and time inputs into LocalDateTime
        LocalDate date = LocalDate.parse(appointmentDate);                // e.g., "2024-11-23"
        LocalTime startLocalTime = LocalTime.parse(startTime);            // e.g., "08:00"
        LocalTime endLocalTime = LocalTime.parse(endTime);                // e.g., "09:02"
        
        
        
        // Convert to Timestamp for database compatibility
        Timestamp startTimestamp = Timestamp.valueOf(LocalDateTime.of(date, startLocalTime));
        Timestamp endTimestamp = Timestamp.valueOf(LocalDateTime.of(date, endLocalTime));
        ...
        int successful = appointment.addAppointment(startTimestamp, endTimestamp, roomId, equipmentIds, date);
